...
Metric configs may also use `timezoneMode: 'floating' | 'fixed'`. Floating metrics evaluate schedules and due-by checks in the client/device local time when client timezone metadata is available; fixed metrics use the Apps Script/server timezone. Physical sheet writes still use the existing script-timezone column model.
