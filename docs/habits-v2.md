    ...
    - Metric Dictionary Structure and Properties:
        ...
        [
            ...
            taskInsightSettings: {insightChance: .9, dayToDayChance: .1, dayToAvgChance: .4, rawValueChance: .1, increaseGood: true, insightUnits: "lbs"},
        		...
        		ifTimer_Settings: {timerMessageTemplate: null, timerStartTaskID: null, timerDurationTaskID: null} // || Timers are now combined with metrics, with start_timer and stop_timer being types of metrics.
        	}
        ]
        ```
...
- **Notion Database Properties to be Updated:** upon a task being logged via iOS, the apps script code uses the Notion API to find a task with a matching taskID, and marks all relevant data to various properties, depending on what the user has configured in the metric settings. Notion Properties include:
    ...
    - streak (apps script sets “Streak” to the calculated_streak_count). This is done for every metric with a task.streaks.sheetID value
    ...
    - [unused by apps script] data (text) → user inputs all other metrics associated with a metric, separated by the separator character (initially Ù) before marking complete. For meditation, it could look like 15:54Ù7.5Ù8.5. This is used to send the metrics payload along with the taskID “key” in the webhooks request to apps script
...
- one shortcut logs every metric by taking in a list of taskID’s, finding them via a locally cached version of the config file, and then dynamically showing menus and prompts for the information needed.
